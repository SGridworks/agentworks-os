...
Stop the conflicting process, or edit `~/.agentworks/docker-compose.yml` to change the host port mapping before reinstalling.
...
1. **AgentWorks OS isn't running.** Run `docker compose ps` and confirm `agentos-d` shows `Up`. If not, run `docker compose up -d`.

2. **Wrong URL in Claude Desktop config.** The MCP server URL in your Claude Desktop config must exactly match the port the daemon is listening on. Check `~/.agentworks/docker-compose.yml` for the host port mapping. By default the daemon listens on port 7710 inside the container, mapped to 7710 on the host. No trailing slash.
...
4. **Firewall blocking port 7710.** The machine running AgentWorks OS needs to accept inbound connections on port 7710.
