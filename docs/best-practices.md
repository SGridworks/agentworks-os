...
```bash
# Restart agentos-d without interrupting in-flight operations
docker compose -f ~/.agentworks/docker-compose.yml restart agentos-d
```
...
```bash
docker compose -f ~/.agentworks/docker-compose.yml pull
docker compose -f ~/.agentworks/docker-compose.yml up -d
```
...
```bash
docker compose -f ~/.agentworks/docker-compose.yml logs scanner-worker | grep "watching"
```
