...
```text
POST http://localhost:7710/api/actions
```
...
```bash
docker compose -f ~/.agentworks/docker-compose.yml logs agentos-d
```
...
docker compose -f ~/.agentworks/docker-compose.yml ps
docker compose -f ~/.agentworks/docker-compose.yml logs scanner-worker
curl http://localhost:7710/api/health
```
