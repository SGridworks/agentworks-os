...
If `~/.agentworks/docker-compose.yml` doesn't exist, this host doesn't have AgentWorks installed. Stop and ask the operator which host they meant.
...
docker compose -f $HOME/.agentworks/docker-compose.yml exec -T agentos-d \
  agentos backup --output /app/data/backups/preop-$(date +%Y%m%d-%H%M%S).tar.gz
...
```bash
docker compose -f $HOME/.agentworks/docker-compose.yml ps --format "{{.Service}} {{.Status}}"
```

**Verify:** `agentos-d`, `scanner-worker`, `n8n` all show `Up`.
...
```bash
docker compose -f $HOME/.agentworks/docker-compose.yml ps
docker compose -f $HOME/.agentworks/docker-compose.yml logs --tail 100 <service>
```
...
```bash
docker compose -f $HOME/.agentworks/docker-compose.yml logs --tail 200 agentos-d
```
...
If logs end at "Listening on :7710" but health still won't answer, port-mapping is wrong. `docker port agentos-d` should show `7710/tcp -> 0.0.0.0:7710`.
...
```bash
docker compose -f $HOME/.agentworks/docker-compose.yml images agentos-d
...
```bash
docker compose -f $HOME/.agentworks/docker-compose.yml stop agentos-d
```

Restore from the most recent good backup:

```bash
docker compose -f $HOME/.agentworks/docker-compose.yml run --rm agentos-d \
  agentos restore --input /app/data/backups/<latest-backup>.tar.gz
```

The v0.1.1 restore tool clears stale `-wal`/`-shm` sidecars before writing. If the restore tool says the source backup's row counts don't match what the tarball contains, the tarball is bad — try the next-most-recent.

Restart:

```bash
docker compose -f $HOME/.agentworks/docker-compose.yml up -d agentos-d
```
...
# Tail daemon
docker compose -f $HOME/.agentworks/docker-compose.yml logs -f agentos-d
