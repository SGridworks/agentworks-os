# AgentWorks `packages/awcp`

AgentWorks Compliance Protocol (AWCP) reference implementation. TypeScript exports of the canonical action schema, policy check API client, and schema mapping utilities. This is the wire-format contract between agents and the policy engine.

## Your lane

- **BackendEngineer** — `^packages/awcp/`
- **TechLead** (escalated) — schema changes require RFC sign-off

## Public API surface

Other packages may import:

- `src/schema.ts` — canonical action schema (Zod + JSON schema export)
- `src/client.ts` — policy check API client
- `src/mapping.ts` — schema mapping utilities

## Do not edit

- `rule-packs/**` (ComplianceConsultant owns YAML content)
- `packages/policy-engine/**` (consume evaluator; don't duplicate logic)
