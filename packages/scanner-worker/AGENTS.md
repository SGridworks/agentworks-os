# AgentWorks `packages/scanner-worker`

Python/FastAPI sidecar that wraps the Apache-2.0 `agentguard-scanner` package. Exposes an internal HTTP API for `agentos-d` to request security posture scans of agent configs (CLAUDE.md, .cursorrules, MCP configs) and returns structured findings.
...
- `packages/agentos-d/**` (BackendEngineer — TS caller)
...
- §6 — Repo-boundary violations. scanner-worker is a NEW package in agentworks-os, NOT a fork of agentguard-scanner.
- §15 — Cross-cutting destructive edits. Don't patch upstream agentguard logic here; contribute upstream.
