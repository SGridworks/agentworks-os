...
import {
  cancelAutomationRun,
  createAutomationRunEvidencePack,
...
function defaultParams(type: AutomationStep['type']): Record<string, unknown> {
  ...
  if (type === 'approval.wait') {
    return {
      proposedActionKind: 'workflow.approval',
      proposedActionSummary: 'Approve workflow continuation',
      decisionReason: 'Workflow pause point requested operator approval.',
    };
  }
  if (type === 'dispatch') {
    return { taskKind: 'workflow.dispatch', targetAgentId: '', input: { source: 'visual-editor' } };
  }
  if (type === 'handoff.contract') {
    return {
      taskKind: 'workflow.handoff',
      targetAgentId: '',
      contract: {
        objective: 'Complete this workflow handoff and return evidence.',
        requiredOutputs: ['summary', 'evidence'],
        acceptanceCriteria: ['requested work complete', 'evidence attached'],
      },
    };
  }
  ...
  if (type === 'evidence.pack') return { include: ['run', 'steps', 'approvals', 'dispatches'] };
  if (type === 'workflow.self_heal') return { monitor: { window: '24h', signals: ['failed_runs', 'stale_dispatch'] } };
  return {
    key: 'automations/visual-editor',
    body: '# Visual Editor Automation\n\nCreated in AWOS.',
    mode: 'append',
...
export default function AutomationsV2() {
  ...
  const [enginePanel, setEnginePanel] = useState<{ title: string; body: string } | null>(null);
  ...
  async function installTemplate(templateId: string) {
    try {
      await installAutomationTemplate(templateId, { tenantId: TENANT_ID, companyId: COMPANY_ID });
      await load();
    } catch (e) {
  ...
  async function simulateWorkflow(workflowId: string) {
    try {
      const result = await simulateAutomationWorkflow(workflowId, {
        source: 'admin-ui',
        companyId: COMPANY_ID,
        requestedAt: new Date().toISOString(),
      });
        ...
        body: JSON.stringify(
          {
            ...
            sideEffectsSuppressed: result.sideEffectsSuppressed,
      ...
      setNotice(`Simulation recorded for ${result.wouldRun.length} step(s).`);
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to simulate workflow');
    }
  }

  async function showWorkflowVersions(workflowId: string) {
    try {
      const versions = await listAutomationWorkflowVersions(workflowId);
      setEnginePanel({
        title: 'WORKFLOW VERSIONS',
        body: JSON.stringify(
          versions.items.map((version) => ({
            version: version.version,
            hash: version.definitionHash.slice(0, 12),
            steps: version.definition.steps.length,
      ...
      setError(e instanceof Error ? e.message : 'Failed to load workflow versions');
    }
  }

  async function selfHealWorkflow(workflowId: string) {
    try {
      const proposal = await selfHealAutomationWorkflow(workflowId);
      setEnginePanel({ title: 'SELF-HEAL PROPOSAL', body: JSON.stringify(proposal, null, 2) });
      setNotice('Self-heal proposal generated without changing the workflow.');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to create self-heal proposal');
    }
  }

  async function replayRun(runId: string) {
    try {
      const run = await replayAutomationRun(runId, 0, { replayedFromUi: true });
      setNotice(`Replay started as ${run.id}.`);
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to replay run');
    }
  }
  ...
  async function resumeRun(runId: string) {
    try {
      const run = await resumeAutomationRun(runId, {
        ...
        reviewedByLabel: 'Local Admin',
      });
      setNotice(`Run ${run.id} resumed with status ${run.status}.`);
      await load();
    } catch (e) {
  ...
  async function cancelRun(runId: string) {
    try {
      const run = await cancelAutomationRun(runId, 'cancelled_from_admin_ui');
      setNotice(`Run ${run.id} cancelled.`);
      await load();
    } catch (e) {
  ...
  async function createEvidencePack(runId: string) {
    try {
      const pack = await createAutomationRunEvidencePack(runId);
      setEnginePanel({ title: 'EVIDENCE PACK', body: pack.markdown });
      setNotice(`Evidence pack ${pack.id} created.`);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to create evidence pack');
    }
  }
        ...
        {enginePanel && (
          <div className="card" style={{ padding: 14, display: 'grid', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
              <div className="eyebrow" style={{ margin: 0 }}>{enginePanel.title}</div>
              <button className="btn btn-sm" onClick={() => setEnginePanel(null)}>Close</button>
            </div>
            <pre
              className="mono"
              style={{
                ...
                <tr key={workflow.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      <button className="btn btn-sm" disabled={loading} onClick={() => simulateWorkflow(workflow.id)}>
                        Simulate
                      </button>
                      <button className="btn btn-sm" disabled={loading} onClick={() => showWorkflowVersions(workflow.id)}>
                        Versions
                      </button>
                      <button className="btn btn-sm" disabled={loading} onClick={() => selfHealWorkflow(workflow.id)}>
                        Self-heal
                      </button>
                    </div>
                  </td>
            ...
            <table className="tbl">
              ...
              <tbody>
                {(status?.recentRuns ?? []).map((run) => (
                  <tr key={run.id}>
                    <td>
                      ...
                      {run.terminalReason && <div className="mono" style={{ fontSize: 10, color: 'var(--ink-4)', marginTop: 3 }}>{run.terminalReason}</div>}
                      {(run.waitingForApprovalId || run.waitingForDispatchId) && (
                        <div className="mono" style={{ fontSize: 10, color: 'var(--accent)', marginTop: 3 }}>
                          waiting · {run.waitingForApprovalId ?? run.waitingForDispatchId}
                        </div>
                      )}
                    </td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        {(run.status === 'waiting_approval' || run.status === 'waiting_dispatch') && (
                          <button className="btn btn-sm" onClick={() => resumeRun(run.id)}>Resume</button>
                        )}
                        {['running', 'waiting_approval', 'waiting_dispatch', 'paused'].includes(run.status) && (
                          <button className="btn btn-sm" onClick={() => cancelRun(run.id)}>Cancel</button>
                        )}
                        <button className="btn btn-sm" onClick={() => replayRun(run.id)}>Replay</button>
                        <button className="btn btn-sm" onClick={() => createEvidencePack(run.id)}>Evidence</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {status && status.recentRuns.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ padding: 24, textAlign: 'center', color: 'var(--ink-4)', fontStyle: 'italic' }}>
