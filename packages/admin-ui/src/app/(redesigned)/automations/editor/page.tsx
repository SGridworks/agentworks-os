...
import { Activity, ArrowLeft, Download, Maximize2, Play, Plus, RefreshCw, Save, Sparkles, Trash2, Workflow, ZoomIn, ZoomOut } from 'lucide-react';
...
const STEP_GROUPS: Array<{ label: string; types: AutomationStep['type'][] }> = [
  ...
  {
    label: 'AWOS ACTIONS',
    types: ['policy.check', 'approval.enqueue', 'approval.wait', 'vault.read', 'vault.write', 'issue.create', 'issue.update', 'dispatch', 'handoff.contract', 'scanner.finding', 'webhook.response'],
  },
  {
    label: 'LOGIC',
...
export default function AutomationsEditorPage() {
  ...
  async function simulateWorkflowNow() {
    if (!workflowId) {
      await saveWorkflow();
      return;
    }
    try {
      ...
      setNotice(`Simulation completed · ${result.wouldRun.length} step(s), ${result.sideEffectsSuppressed.length} side effect(s) suppressed.`);
      await load();
    } catch (e) {
  ...
  async function draftWithAi() {
    ...
    setError(null);
    try {
      const result = await draftAutomationTemplate({ tenantId: TENANT_ID, companyId: COMPANY_ID, prompt: aiPrompt }) as {
      ...
      <div style={{ height: '100%', display: 'grid', gridTemplateRows: 'auto minmax(0,1fr)', background: 'var(--bg)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '240px minmax(520px,1fr) 360px', minHeight: 0 }}>
          <aside style={{ borderRight: '1px solid var(--rule)', padding: 16, display: 'grid', gap: 12, alignContent: 'start', overflowY: 'auto' }}>
            ...
            <button className="btn btn-sm" onClick={simulateWorkflowNow} disabled={!workflowId}>
